...
ConcurrentHashMap 和 HashMap 实现上类似，**<u>最主要的差别是 ConcurrentHashMap 采用了分段锁（Segment）</u>**，每个分段锁维护着几个桶（HashEntry）**，多个线程可以同时访问不同分段锁上的桶**，从而使其并发度更高（并发度就是 Segment 的个数）。
...
***在执行 size 操作***时，需要遍历所有 Segment 然后把 count 累计起来。
